...
Wrapper is a utility component that can wrap it's content in a specific width depending on if the application has a full or fixed width.

If not used with `fullWidth` it centers content horizontally and ensures that it doesn't stretch indefinitely on larger screens.
